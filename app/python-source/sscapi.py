...
class PySSC:
	...
	def __init__(self):
#		print("initializing pyssc")
		if sys.platform == 'win32' or sys.platform == 'cygwin':
			if 8*struct.calcsize("P") == 64:
				self.pdll = CDLL(os.path.join(basepath, "../../sdk-win64/ssc.dll") )
			else:
				self.pdll = CDLL(os.path.join(basepath, "../../sdk-win32/ssc.dll") )
		elif sys.platform == 'darwin':
			self.pdll = CDLL("ssc.dylib") 
		elif sys.platform in ['linux2', 'linux']:
			self.pdll = CDLL(os.path.join(basepath, "../../sdk-linux/ssc.so"))   # instead of relative path, require user to have on LD_LIBRARY_PATH
		else:
